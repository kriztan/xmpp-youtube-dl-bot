...
# changes by kriztan 10/2022
# rewrite for Python3
# use yt_dlp instead of youtube_dl
# httpupload feature not working atm 
...
def messageCB(sess, mess):
    ...
    # and send it when yt_dlp returns.
...
def start_upload(sess, to, file_obj):
    ...
    req.setAttr('filename', file_obj['filename'])
    req.setAttr('size', os.path.getsize(file_obj['filepath']))
    req.setAttr('content-type', file_obj.get('content-type', 'image/jpeg'))
